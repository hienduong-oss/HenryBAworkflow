...
This command is deprecated for current BA-kit flow. ASCII wireframes are mandatory inside `paths.screen_root` screen canon files and are generated by `ba-start srs`.
...
- Require `paths.srs_index`, `paths.screen_root`, and `paths.srs_compile_receipt`.
