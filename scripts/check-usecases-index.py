#!/usr/bin/env python3
"""Validate usecases/index.md for a module against the new folder-based canon."""
...
def main() -> int:
    ...
    # Verify each UC file listed in the index actually exists
    if result["state"] == "current" and "rows" in result:
        ...
        if missing_files:
            result["state"] = "stale"
            result["reason"] = "usecase_files_missing"
    ...
    if result["state"] == "current":
        return 0
    print(
        f"USECASES_INDEX_FAIL: state={result['state']} reason={result.get('reason', '')}",
        file=sys.stderr,
    )
    return 1
