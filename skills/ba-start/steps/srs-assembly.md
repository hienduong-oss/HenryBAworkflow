...
3. After all groups are appended, run a cross-artifact consistency check on the file on disk:
   ...
   - run `python3 scripts/validate-navigation-consistency.py --design {paths.design_doc} --screen-contract {paths.srs_group group=c}` when UI-backed screens and `paths.design_doc` exist; treat any `NAV_SCHEMA_MISMATCH`, `MENU_SCHEMA_MISMATCH`, or `MENU_ACTIVE_MISSING` as blocking
