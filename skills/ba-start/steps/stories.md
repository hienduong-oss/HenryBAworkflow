...
**Story quality rules (BLOCKING):**
- Generic personas (`user`, `admin`, `member`) are NOT acceptable — use role-specific personas.
- Every story MUST pass all 6 INVEST criteria before being written.
- Every story MUST have minimum 3 AC: happy path, edge case, negative path.
- Each AC MUST use Gherkin Given-When-Then with measurable outcomes.
- No vague qualifiers in AC ("fast", "user-friendly", "safe") — use concrete thresholds.
