...
- May read: targeted `paths.backbone`, targeted user story items, `paths.usecases_index` and `paths.ascii_screen_index` when refreshing, `paths.project_memory` or hot shards, module warm shard, and `paths.frd` when it exists.
- Must not read: `log.md`, `cold/`, other module shards, full `paths.backbone`, or all user story items when indexes are current.
...
When SRS writes or refreshes `paths.usecases_index` or `paths.ascii_screen_index`, generate it with `stale_status: unknown`, leave `validated_at` and `validated_by` blank, and run producer-side validation before any downstream routing treats the index as current.
...
Run `python3 scripts/validate-navigation-consistency.py --design {paths.design_doc} --screen-contract {paths.screen_field_contract}` after screen contract generation and before final screen assembly when UI-backed screens exist. Treat `NAV_SCHEMA_MISMATCH`, `MENU_SCHEMA_MISMATCH`, and `MENU_ACTIVE_MISSING` as blocking.
